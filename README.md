...
- `tools/ground_question_answers.mjs`：按课程材料、整理纪要、重绘图和 PPT/往年题资料校正真题答题框架，并给每题写入维护用来源字段；可用 `--check` 验证题库仍与校对脚本一致。
