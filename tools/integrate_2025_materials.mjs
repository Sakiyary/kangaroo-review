...
const newQuestions = [
  {
    ...
    answer_frame_en: "Use the same six-part scenario template.\nFor operability, focus on operator actions, diagnosis, configuration, rollback, or monitoring.\nFor performance, focus on workload, latency/throughput, and measurable response time.",
    question_en: "Describe operability and performance quality attribute scenarios.",
    answer_frame_zh: "仍用六要素模板。\n可操作性写操作者在某环境下执行诊断、配置、回滚或监控操作，系统给出可理解、可执行的响应，并用完成时间/错误率/恢复时间度量。\n性能写用户或外部系统在峰值/正常负载下发起请求，目标制品在限定延迟、吞吐或资源占用内响应。",
    ...
    sample_answer_zh: "可操作性场景：在系统正常运行且订单服务告警时，运维人员通过监控控制台查看异常订单链路。\n可操作性响应：系统应展示相关日志、指标和依赖服务状态，并允许一键回滚到上一个稳定版本。\n可操作性响应度量：2 分钟内定位故障服务，5 分钟内完成回滚，且回滚过程不影响已支付订单。\n性能场景：在晚高峰每秒 1000 个下单请求下，用户向订单服务提交订单，系统应完成校验、创建订单并返回结果。\n性能响应度量：95% 请求在 2 秒内返回，错误率低于 0.1%。",
    sample_answer_en: "Operability scenario: during normal operation, an operator receives an order-service alarm and uses the console to inspect traces, logs, metrics, and dependencies.\nOperability response: the system shows relevant logs, metrics, and dependency status and allows rollback to the last stable version.\nOperability response measure: diagnosis within 2 minutes and rollback within 5 minutes without affecting paid orders.\nPerformance scenario: under 1000 order requests per second at peak time, the user submits an order and the system validates, creates, and returns the order result.\nPerformance response measure: 95% of requests return within 2 seconds and error rate stays below 0.1%.",
  ...
  },
  {
    ...
    answer_frame_en: "Define microservices as small independently deployable services organized around business capabilities.\nExplain service autonomy, decentralized governance/data, lightweight communication, and automated infrastructure.\nFinish with distributed-system tradeoffs.",
    question_en: "Describe the main characteristics of microservice architecture.",
    answer_frame_zh: "微服务把单体应用拆成围绕业务能力组织的小服务。\n每个服务可独立开发、测试、部署和扩展，服务之间通过轻量通信协作。\n团队和技术治理更去中心化，服务通常拥有自己的数据边界，并依赖 CI/CD、容器、服务发现、监控和链路追踪等自动化基础设施。\n代价是网络失败、分布式事务、一致性和运维复杂度上升。",
    ...
    sample_answer_zh: "服务组件化：把业务能力封装为可独立替换和升级的服务。\n围绕业务能力组织：系统边界和团队边界都尽量按业务能力划分。\n高内聚低耦合：服务内部保有领域逻辑，服务间减少直接依赖。\n去中心化治理和数据：不同服务可选合适技术栈，并拥有自己的数据库或数据边界。\n基础设施自动化：依赖 CI/CD、容器、服务发现、监控和链路追踪支撑频繁交付。\n独立演进与容错：服务可以独立演进，并通过高可用设计容忍局部失败。",
    sample_answer_en: "Componentization via services: package each business capability as an independently replaceable and upgradeable service.\nOrganization around business capabilities: align service boundaries and team boundaries with business capabilities.\nHigh cohesion and loose coupling: keep domain logic inside services and reduce direct inter-service dependencies.\nDecentralized governance and data: services may choose suitable technology stacks and own their databases/data boundaries.\nInfrastructure automation: use CI/CD, containers, service discovery, monitoring, and tracing for frequent delivery.\nIndependent evolution and resilience: services evolve independently and tolerate local failures through resilient design.",
  ...
  },
  {
    ...
    answer_frame_en: "Explain why the scenario fits microservices.\nIdentify operations from verbs.\nMap business capabilities/subdomains to services.\nDescribe APIs/events, data ownership, and quality tradeoffs.",
    question_en: "Design a food-delivery platform using microservices and explain the service decomposition.",
    answer_frame_zh: "先说明外卖平台业务域多、流量峰值明显、团队可按业务能力并行演进，因此适合微服务。\n再从动词识别系统操作：浏览菜单、下单、支付、接单、配送、通知、结算。\n服务可拆为用户、商家/菜单、订单、支付、配送调度、通知、结算/评价等。\n每个服务拥有自己的数据，跨服务用 API Gateway、同步查询和异步领域事件协作，并用 Saga/补偿保证最终一致性。",
    ...
    diagram_id: "microservice-style-comparison",
    sample_answer_zh: "适用原因：外卖平台适合微服务，因为用户、商家、订单、支付、骑手调度和通知属于不同业务能力，访问压力和变化频率不同，可以独立扩展和迭代。\n系统操作：浏览餐厅和菜单、创建订单、支付订单、商家接单、分配骑手、更新配送状态、发送通知和结算。\n服务拆分：可以设计为 User Service、Restaurant/Menu Service、Order Service、Payment Service、Delivery/Dispatch Service、Notification Service、Settlement Service。\n协作方式：用户请求先进入 API Gateway；Order Service 创建订单后发布 OrderCreated 事件，Payment Service 支付成功后发布 PaymentSucceeded，Delivery Service 订阅后分配骑手并更新状态。\n一致性：订单、支付、配送分别拥有自己的数据库，跨服务一致性用 Saga 和补偿操作处理。",
    sample_answer_en: "Fit reason: a food-delivery platform fits microservices because user, restaurant/menu, order, payment, delivery, notification, and settlement capabilities evolve and scale differently.\nSystem operations: browse menus, create orders, pay, accept orders, dispatch riders, update delivery status, notify, and settle.\nService split: use User Service, Restaurant/Menu Service, Order Service, Payment Service, Delivery/Dispatch Service, Notification Service, and Settlement Service.\nCollaboration: requests enter through an API Gateway; Order Service publishes OrderCreated, Payment Service publishes PaymentSucceeded, and Delivery Service subscribes and updates delivery status.\nConsistency: order, payment, and delivery own separate databases, and cross-service consistency uses Saga/compensation.",
